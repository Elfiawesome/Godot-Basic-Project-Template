...
func _ready() -> void:
	...
	if global._instance_num == 1: get_window().position = Vector2(DisplayServer.screen_get_size().x/2.0,0)
	if global._instance_num == 2: get_window().position = Vector2(0,DisplayServer.screen_get_size().y/2.0)
	if global._instance_num == 3: get_window().position = DisplayServer.screen_get_size()/2.0
